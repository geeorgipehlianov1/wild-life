...
const URL_PATTERTN = /^https?:\/\/(.+)/

const postSchema = new Schema({
    title: { type: String, minlength: [6, 'Title must be at least 6 characters long'] },
    keyword: { type: String, minlength: [6, 'Keyword must be at least 6 charcters long'] },
    location: { type: String, maxlength: [15, 'Location must be at most 15 characters long'] },
    date: { 
        type: String,
        minlength: [10, 'Title must be al least 10 characters long'],
        maxlength: [10, 'Title must be al most 10 characters long'] },
    image: { type: String, validate: {
        validator(value) {
            return URL_PATTERTN.test(value);
        },
